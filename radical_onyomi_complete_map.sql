drop table if exists #radical_onyomi_complete_map
declare @cutoff int = 10,@cutoff2 int = 5
;with kanji_radicals_et_eles as(select * from kanji_radicals_et_elements where radicals not in
...
select kr.radicals+','
		...
		radicals
		,r.ja_on
		,count(distinct r.literal) kanji_count
		,left(STRING_AGG(r.literal,'')within group (order by len(r.meanings),r.literal),20)examples
	into #radical_onyomi_complete_map
	from kanji_radicals_et_eles as kr
	join kanjidic2_readings r on kr.kanji=r.literal and r.ja_on is not null
	left join kanji_radicals_et_eles kr2 on kr2.radicals>kr.radicals and kr.kanji=kr2.kanji
	left join kanji_radicals_et_eles kr3 on kr3.radicals>kr2.radicals and kr2.kanji=kr3.kanji
	left join kanji_radicals_et_eles kr4 on kr4.radicals>kr3.radicals and kr.kanji=kr4.kanji
	left join kanji_radicals_et_eles kr5 on kr5.radicals>kr4.radicals and kr.kanji=kr5.kanji
	left join kanji_radicals_et_eles kr6 on kr6.radicals>kr5.radicals and kr.kanji=kr6.kanji
	left join kanji_radicals_et_eles kr7 on kr7.radicals>kr6.radicals and kr.kanji=kr7.kanji
	left join kanji_radicals_et_eles kr8 on kr8.radicals>kr7.radicals and kr.kanji=kr8.kanji
	left join kanji_radicals_et_eles kr9 on kr9.radicals>kr8.radicals and kr.kanji=kr9.kanji
	left join kanji_radicals_et_eles kr10 on kr10.radicals>kr9.radicals and kr.kanji=kr10.kanji
	group by 
	kr.radicals,isnull(kr2.radicals+',','')
		...
		,isnull(kr10.radicals,'')
	,r.ja_on
;with cte as (select case when right(radicals,1)=','then left(radicals,len(radicals)-1)else radicals end radicals,ja_on,kanji_count,examples from #radical_onyomi_complete_map m where exists (select 1 from #radical_onyomi_complete_map m1 where m1.radicals=m.radicals and m1.ja_on=m.ja_on group by m1.radicals,m1.ja_on having sum(m1.kanji_count)>=@cutoff2))
...
select p.radicals,p.ja_on,p.examples,p.given_rad_chance_of_on,p.given_on_chance_of_rad
from yomi_and_radical_pattern p
----where not exists
----(select 1 from yomi_and_radical_pattern q 
----where q.radicals like p.radicals+'%'and p.radicals!=q.radicals
----and q.examples=p.examples 
----and q.ja_on=p.ja_on 
----and q.given_rad_chance_of_on=p.given_rad_chance_of_on)
------
--;with a as(select 'a,b,c,d,e' a),b as(select 'a,b,c,d,f,g,e' b)
--select a.a y,from a
--where len(replace(a.a,',',''))!=
--(select count(*) from a aa cross apply string_split(a.a,',')x
--join b on 1=1
--cross apply string_split(b.b,',')y
--where a.a=aa.a
--and y.value=x.value
--)
where len(replace(p.radicals,',',''))!=
(select count(distinct x.value) from yomi_and_radical_pattern pp cross apply string_split(pp.radicals,',')x
join yomi_and_radical_pattern qq on qq.examples=pp.examples and qq.ja_on=pp.ja_on and qq.given_rad_chance_of_on=pp.given_rad_chance_of_on and len(qq.radicals)>len(pp.radicals)--idk why writing qq.radicals!=pp.radicals doesn't get same results
cross apply string_split(qq.radicals,',')y
where p.examples=pp.examples and p.ja_on=pp.ja_on and p.given_rad_chance_of_on=pp.given_rad_chance_of_on and p.radicals=pp.radicals
and x.value=y.value
)
order by p.radicals,p.given_on_chance_of_rad,p.given_rad_chance_of_on,p.ja_on,p.examples
